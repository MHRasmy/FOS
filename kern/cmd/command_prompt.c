...
str2lower(temp,arguments[0]);
		...
		for (int i =0 ; i < NUM_OF_COMMANDS;i++){
			...
			int len = strlen(commands[i].name);
		         ...
		         if (strncmp(temp,commands[i].name , len) == 0){
		         ...
		         else if(str_str_match(temp,commands[i].name)){
		        	 		LIST_INSERT_TAIL(&foundCommands,&commands[i]);
		        	 		bool_check = 1;
		         }
...
int str_str_match(char * user_string,char* true_string){
	int user_string_len = strlen(user_string);
	    int true_string_len = strlen(true_string);
	    int check = 0;

	    for (int i =0 ; i<user_string_len;i++){

	        for(int j = 0; j< true_string_len; j++){

	            if (user_string[i] == true_string[j]){
	                check = 1;
	                i++;
	                if(i == user_string_len)
	                    break;
	            }
