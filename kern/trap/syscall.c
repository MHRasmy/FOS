...
/*******************************/
int get_permition(uint32 va){
	int per = pt_get_page_permissions(curenv->env_page_directory, va);
	return per;
}
...
void* sys_sbrk(int increment)
{
	...
	//	return (void*)-1 ;
		...
		bool isNeg = increment < 0;
		increment = isNeg? increment*-1:increment;
		increment = isNeg? ROUNDDOWN(increment, PAGE_SIZE) : ROUNDUP(increment, PAGE_SIZE);
		increment = isNeg? increment*-1:increment;
		uint32 ret = env->brk;

		if ((env->brk + increment) >= env->hardLimit)
			return (void*)-1 ;
		...
		else if (increment > 0)
		{
			uint32 va = env->brk;
			env->brk += increment;
			uint32 noPages = increment / PAGE_SIZE;
			...
			{
				pt_set_page_permissions(env->env_page_directory, va, PERM_AVAILABLE, 0);
				//Updating the virtual address
				va += PAGE_SIZE;
			}
		}
		...
		else if (increment < 0)
		{
			...
			for (uint32 i = 0; i < noPages; ++i)
			{
				...
				pt_set_page_permissions(env->env_page_directory, va, 0, PERM_AVAILABLE);


				//Free a frame
				uint32 *ptr_page_table1 = NULL;
				struct FrameInfo* ptr_frame_info = get_frame_info(env->env_page_directory,va, &ptr_page_table);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case NSYSCALLS:
		return 	-E_INVAL;
		break;
	case SYS_GET_PERMITION:
		return get_permition(a1);
	}
