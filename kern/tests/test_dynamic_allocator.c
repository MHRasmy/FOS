...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//[3.3] reallocate in new place
		cprintf("	3.3: reallocate in new place \n\n") ;
		is_correct = 1;
			{

			va = realloc_block_FF(startVAs[150], 5000);
	...
	//check reallocated block size & status
			block_size = get_block_size(startVAs[1200]) ;
			if (block_size != 5000 + sizeOfMetaData())
			{
				is_correct = 0;
				cprintf("test_realloc_block_FF #11.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",5000 + sizeOfMetaData(), block_size);
			}
			block_status = is_free_block(startVAs[1200]) ;
			if (block_status != 0)
			{
				is_correct = 0;
	...
	//[4.2] next block is free (coalesce)
			...
			is_correct=1;
			{
				...
				va = realloc_block_FF(startVAs[199], 2*kilo);
				//check return address
				if(va == NULL || (va != startVAs[199]))
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #15.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n", startVAs[199] ,va);
				}
				//check reallocated block size & status
				block_size = get_block_size(startVAs[199]) ;
				if (block_size != 2*kilo + sizeOfMetaData())
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #15.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",4 + sizeOfMetaData(), block_size);
				}
				block_status = is_free_block(startVAs[199]) ;
				if (block_status != 0)
				{
					is_correct = 0;
				...
				//check new free block
				struct BlockMetaData *newBlkMetaData = (struct BlockMetaData *)(va + 2*kilo);

				if (newBlkMetaData->size != 2068 || newBlkMetaData->is_free != 1)
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #15.4: WRONG REALLOC! newly created block is not correct... check it!. Expected %d, Actual %d\n", 2068, newBlkMetaData->size);
				}
				//check content of reallocated block
				if (*(startVAs[199]) != 199 || *(midVAs[199]) != 199)
				{
					is_correct = 0;
		...
		//[4.3] no enough space for a new block (no change)
				cprintf("	4.3: no enough space for a new block (no change)\n\n") ;
				is_correct=1;
				{

					va = realloc_block_FF(startVAs[399], 10);
					//check return address
					if(va == NULL || (va != startVAs[399]))
					{
						is_correct = 0;
						cprintf("test_realloc_block_FF #16.1: WRONG REALLOC - it return wrong address. Expected %x, Actual %x\n", startVAs[199] ,va);
					}
					//check reallocated block size & status
					block_size = get_block_size(startVAs[399]) ;
					if (block_size != 20 + sizeOfMetaData())
					{
						is_correct = 0;
						cprintf("test_realloc_block_FF #16.2: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",20 + sizeOfMetaData(), block_size);
					}
					block_status = is_free_block(startVAs[399]) ;
					if (block_status != 0)
					{
						is_correct = 0;
					...
					//check new free block
					struct BlockMetaData *newBlkMetaData = (struct BlockMetaData *)(va + 20);

					if (newBlkMetaData->size==1025)
					{
						is_correct = 0;
						cprintf("test_realloc_block_FF #16.4: WRONG REALLOC! mustn't be any new block");
					}
					//check content of reallocated block
					if (*(startVAs[399]) != 399 || *(midVAs[399]) != 399)
					{
						is_correct = 0;
		...
		//[5] the new size equals the old (no change)
			cprintf("	5: the new size equals the old (no change)\n\n") ;
			is_correct=1;
			{

				va = realloc_block_FF(startVAs[399], 20);
				//check return address
				if(va == NULL || (va != startVAs[399]))
				{
					is_correct = 0;
				...
				{
					is_correct = 0;
					cprintf("test_realloc_block_FF #17.4: WRONG REALLOC! mustn't be any new block");
				}
				//check content of reallocated block
				if (*(startVAs[399]) != 399 || *(midVAs[399]) != 399)
				{
